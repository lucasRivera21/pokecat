...
import com.example.pokecat.utils.Credentials.Companion.MAX_RECOGNITION_CAT_RESULTS
...
class Classifier @Inject constructor(
     ...
     */
    fun recognizeImage(bitmap: Bitmap): List<CatRecognition> {
     ...
     */
    private fun getTopKProbability(labelProb: Map<String, Float>): List<CatRecognition> {
        // Find the best classifications.
        val priorityQueue =
            PriorityQueue(MAX_RECOGNITION_CAT_RESULTS) { lhs: CatRecognition, rhs: CatRecognition ->
                (rhs.confidence).compareTo(lhs.confidence)
            }

        for ((key, value) in labelProb) {
            priorityQueue.add(CatRecognition(key, value * 100.0f))
        }

        val recognitions = mutableListOf<CatRecognition>()
        val recognitionsSize = minOf(priorityQueue.size, MAX_RECOGNITION_CAT_RESULTS)
